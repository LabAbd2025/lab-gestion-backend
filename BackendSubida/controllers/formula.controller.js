...
exports.eliminarFormula = async (req, res) => {
  try {
    ...
    // Eliminar las materias primas asociadas primero
    await MateriaPrima.destroy({ where: { formulaId: id } });

    // Luego eliminar la fórmula
    const eliminado = await Formula.destroy({ where: { id } });

    if (!eliminado) {
      return res.status(404).json({ msg: "Fórmula no encontrada" });
    }

    res.json({ msg: "Fórmula eliminada correctamente" });
  } catch (error) {
    console.error("❌ Error al eliminar fórmula:", error);
    res.status(500).json({ msg: "Error al eliminar fórmula", error: error.message });
  }
};


exports.editarFormula = async (req, res) => {
  try {
    const id = req.params.id;
    const { productoNombre, volumenNominal, materiasPrimas } = req.body;
    ...
    // Actualizar fórmula
    await formula.update({ productoNombre, volumenNominal });
    ...
    // Devolver la fórmula actualizada con materias
    const resultado = await Formula.findByPk(id, {
      include: [{ model: MateriaPrima, as: "materiasPrimas" }]
    });

    res.json(resultado);
  } catch (error) {
    console.error("❌ Error al editar fórmula:", error);
    res.status(500).json({ msg: "Error al editar fórmula", error: error.message });
  }
};
